...
public class SocketService {
    ...
    private static ConcurrentHashMap<String, SocketService> webSocketSet = new ConcurrentHashMap<String, SocketService>();
    ...
    public void onOpen(@PathParam(value = "userno") String param, Session session) {
        logger.info("============================userno:{}", param);
        userno = param;//接收到发送消息的人员编号
        this.session = session;
        webSocketSet.put(param, this);//加入map中
        addOnlineCount();           //在线数加1
        logger.info("有新连接加入！当前在线人数为:{},userno:{}", getOnlineCount(), "userno" + param);
    }
